...
- **Переносимость:** desktop-логирование работает без Pino worker-транспорта,
  а `prepare-runtime.mjs` отклоняет bundle с абсолютными путями компьютера
  сборки. Проверка другого каталога — `bundle-portability.test.mjs`.
