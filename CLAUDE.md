...
## План: десктоп-сборка (установщик, «как настоящее приложение»)

Цель — отдать клиенту `.exe` (флешка → установил/запустил → работает как обычная
программа, без Node/браузера/командной строки). **Делать в самом конце, после
...
- **Плюс текущей версии:** бэкенд на **sql.js (WASM)**, нативных модулей нет —
  ...
  `better-sqlite3`).
- **Как собрать внутри:** билд фронтенда (`pnpm build`) → один Express внутри Electron
  отдаёт статику + `/api` → окно Electron открывает `http://localhost:<порт>`
  (относительный `/api` работает, т.к. один origin). БД — в `app.getPath('userData')`
  или, по желанию, рядом с программой/на флешке.
- **Таргеты `electron-builder`:** `nsis` (установщик-мастер) и/или `portable`
  (один `.exe` без установки — удобно для флешки). macOS — `--mac` (лучше на самом Mac).
...
- **`electron-app/`** в корне — устаревшая заготовка-заглушка (грузит `schedule-app/dist`,
  бэкенд не запускает). При реализации переписать под `artifacts/`, а не доверять ей.
