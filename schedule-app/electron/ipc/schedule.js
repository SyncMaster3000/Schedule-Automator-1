// IPC: конструктор расписания — занятия + проверка накладок в реальном времени
const { getDb } = require("../db");
const { checkScheduleItem, rebuildLocksForItem } = require("../services/conflicts");
...
function listByPeriod(periodId, crossPeriod = false) {
  ...
  // Для каждого занятия считаем конфликты (в пределах периода или сквозь все)
  ...
  "schedule:saveItem": (data) => {
    ...
    if (itemId) {
      ...
      const order =
        (db
          .prepare(
            "SELECT MAX(sort_order) AS m FROM schedule_items WHERE period_id = ?"
          )
