// Диспетчер обработчиков расписания для desktop-версии (Electron).
// Портирован из web-бэкенда (server.js): те же обработчики (бывшие IPC-каналы),
// но вместо Express-эндпоинта их вызывает главный процесс Electron напрямую.
// БД (sql.js) инициализируется лениво и сохраняется на диск после каждой записи.
...
async function initSchedule() {
  // Каталог данных задаётся главным процессом (app.getPath('userData')).
  const dataDir = process.env.SCHEDULE_DATA_DIR;
  if (!dataDir) throw new Error("SCHEDULE_DATA_DIR не задан");
...
// Импорт УТП из выбранного .docx (Buffer) — возвращает превью тем.
...
// Возвращает { buffer, filename, count } — главный процесс сохраняет файл диалогом.
